...
class PogoSession(object):
    ...
    def __str__(self):
        s = 'Access Token: {0}\nEndpoint: {1}\nLocation: {2}'.format(self.accessToken,
    ...
    def walkTo(self, olatitude, olongitude, epsilon=10, step=7.5):
        ...
        dist = closest = getDistance(latitude, longitude, olatitude, olongitude)
        ...
        while dist > epsilon:
            logging.info("%f m -> %f m away", closest - dist, closest)  # Lazy logging
            latitude -= dLat
            longitude -= dLon
            self.setCoords(
                latitude,
